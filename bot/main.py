...
async def handle_body_measurement_image(image_bytes: bytes) -> dict:
    prompt = """На изображении скриншот умных весов. Извлеки все числовые показатели.
Верни ТОЛЬКО JSON без пояснений, в формате:
{
  "weight": число или null,
  "bmi": число или null,
  "fat_percent": число или null,
  "muscle_percent": число или null,
  "water_percent": число или null,
  "visceral_fat": целое число или null,
  "bmr": целое число или null,
  "fat_mass": число или null,
  "lean_mass": число или null,
  "bone_mass": число или null
}
Если показатель не виден — ставь null. Только JSON, никакого текста вокруг."""
...
async def handle_product_image(image_bytes: bytes) -> dict:
    prompt = """На изображении этикетка продукта питания. Извлеки название, бренд и пищевую ценность.
Верни ТОЛЬКО JSON:
{
  "name": "название продукта",
  "brand": "бренд или null",
  "calories": число на 100г,
  "protein": число на 100г,
  "fat": число на 100г,
  "carbs": число на 100г
}
...
async def classify_message(text: str) -> dict:
    system = """Ты классификатор сообщений для системы здоровья и питания.
...
Типы:
- "body_measurement_text" - пользователь описывает замер тела текстом
- "add_product" - добавить продукт в базу (с КБЖУ)
- "daily_goals" - запрос дневных целей питания
- "meal_plan" - составить план питания на день
- "food_log" - записать что съел
- "workout_update" - сообщает о тренировке
- "other" - всё остальное

Для "food_log" извлеки: [{"name": "продукт", "grams": число}, ...]
Для "add_product" извлеки: {"name": "...", "brand": "...", "calories": ..., "protein": ..., "fat": ..., "carbs": ...}
Для "workout_update" извлеки: {"has_workout": true/false}
Для "meal_plan" извлеки: {"products": ["список продуктов"], "meals_count": число, "notes": "примечания"}
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Пробуем определить тип фото
    # Сначала пробуем как замер весов
    try:
    ...
    await update.message.reply_text("Не смог распознать изображение. Отправь скриншот весов или фото этикетки продукта.")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Не понял сообщение. Попробуй ещё раз.")
        return
    ...
    if msg_type == "body_measurement_text":
        ...
        for item in log_items:
            if item.get("calories"):
                ...
                lines.append(f"{item['product_name']}: {item['grams']} г (продукт не найден в базе)")
