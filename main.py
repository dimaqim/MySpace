...
7. УДАЛЕНИЕ: "удали всё / удали все записи / всё кроме X" означает ВСЕ категории сразу — вызови И delete_food, И delete_finance (а если просят и напоминания/задачи — их тоже). Удаляй всё что просили, кроме явно исключённого ("кроме напоминания" → напоминание НЕ трогай). Не оставляй финансы или еду, если человек сказал "всё".
8. Если не уверен что именно удалить — лучше удали более широко то что просили, чем оставь лишнее. Человек всегда может записать заново.
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        raw = await gpt_vision(image_bytes,
            "На изображении скриншот приложения умных весов. Твоя задача — точно извлечь ВСЕ показатели.\n\n"
            "ВАЖНО про дату:\n"
            "• Найди дату замера на экране — она может быть в верхней части, в заголовке, рядом с весом.\n"
            "• Формат на скрине может быть любым: 10 Jun 2026 / 10.06.2026 / 2026-06-10 / Jun 10 / 10/06 и т.д.\n"
            "• Преобразуй найденную дату в формат YYYY-MM-DD (например: 2026-06-10).\n"
            "• КРИТИЧЕСКИ важно: не путай день и месяц. Если видишь '10 Jun' — это 10 июня = 2026-06-10.\n"
            "• Если год не указан на скрине — используй 2026.\n"
            "• Если дату совсем не видно — верни null.\n\n"
            "Соответствие остальных полей:\n"
            "• weight — Вес (кг)\n"
            "• bmi — ИМТ\n"
            ...
            "Если показателя нет на скрине — ставь null. Верни ТОЛЬКО JSON без пояснений:\n"
...
def main():
    ...
    async def remind_xcode_resign(bot: Bot):
        await bot.send_message(chat_id=MY_CHAT_ID, text=(
            "🔧 *Напоминание: обнови приложение в Xcode*\n\n"
            "Сертификат разработчика действует 7 дней\\.\n"
            "Открой Xcode → выбери устройство → нажми ▶ Run\\.\n\n"
            "Иначе LifeOSSync перестанет открываться на iPhone\\."
    ...
    scheduler.add_job(remind_xcode_resign,  "interval", days=6,          hours=12,            args=[app.bot])
    scheduler.start()
