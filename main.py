...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    # ── meal_session: новый приём пищи с несколькими продуктами ──
        ...
        # general_chat — отвечаем только по теме личного трекера
        stats = get_today_stats()
        reply = await gpt(
            f"Ты личный трекер-ассистент. Твоя задача — помогать пользователю отслеживать питание, вес, тренировки, финансы и привычки.\n"
            f"Данные пользователя за сегодня:\n{stats}\n\n"
            f"ПРАВИЛА:\n"
            f"- Отвечай кратко, по-русски, без лишних слов\n"
            f"- Если вопрос про питание/вес/тренировки/финансы — отвечай используя данные выше\n"
            f"- Если вопрос не по теме (новости, погода, политика, общие знания) — скажи коротко: 'Я только твой личный трекер. Могу помочь с едой, весом, тренировками или финансами.'\n"
            f"- Никаких ссылок, никаких списков новостных сайтов\n"
            f"- Не пиши смайлики если не спрашивают",
            text
        )
        await update.message.reply_text(reply)
